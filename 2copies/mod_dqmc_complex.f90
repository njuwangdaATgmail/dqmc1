!>========================================================================
!! This module is designed as the core of a general single-component     !
!! DQMC program.                                                         !
!!                                                                       !
!! The single particle "action" is:                                      !
!!   c'*K*c + lam*c'*F*c + phi*c'*F*c                                    !
!!            descrete     continuous                                    !
!! Then, the definition of B is:                                         !
!!   B = exp(K)*exp(phi(nphi)*F(nphi))*...*exp(phi(1)F(1))               !
!!             *exp(lam(nising)F(nising))*...*exp(lam(1)F(1))            !
!! where K is -dtau*H0.                                                  !
!!                                                                       !
!! List of external subroutines to be specified by users:                !
!!   init(): define necessary parameters listed below                    !
!!   acceptprob_ising(ratio,newising,site,time,ifield)                   !
!!   acceptprob_phi(ratio,newphi,site,time,ifield)                       !
!!   generate_newising_global(ifield)                                    !
!!   generate_newphi_global(ifield)                                      !
!!   acceptprob_ising_global(ratio,newising,ifield)                      !
!!   acceptprob_phi_global(ratio,newphi,ifield)                          !
!!   measurement(time)                                                   !
!!   postprocess()                                                       !
!!                                                                       !
!! This DQMC module is used by externally calling                        !
!!   dqmc_driver()                                                       !
!!                                                                       !
!! Parameter list should be defined in init() by users from outside:     !
!!   restart,proj,nsite,nelec,ntime,nsp,
!!   nising,isingmax,form_ising,lam_ising
!!   nphi,dphi,form_phi,mask_ising,mask_phi
!!   nform,ndim_form,mask_form,nb_form,fmat
!!   nbin,nwarmup,nmeasure,ninterval,ntmpout,nscratch,ngroup,
!!   randomseed,nglobal_ising,nglobal_phi,poolsize_r,poolsize_z
!!   expk,expk_half,inv_expk,inv_expk_half: via set_expk(kmat,dtau)
!!   slater: via set_slater(kmat,dtau)
!>========================================================================
MODULE dqmc_complex
...
CONTAINS
  ...
  SUBROUTINE dqmc_driver()
    ...
    CALL average_pool() ! do average of the measured observables in the pool
    CALL postprocess()  ! external subroutine, data analysis and output
  ...
  SUBROUTINE dqmc_update(do_meas)
    IMPLICIT NONE
